...
function validateMessage(message) {
  ...
  if (message.country === undefined) {
    warnings.push('Missing "country" field (null is OK for localhost)');
  }
...
async function runTests() {
  ...
  log(
    `RPC Endpoint: http://${CONFIG.host}:${CONFIG.port}${CONFIG.rpcEndpoint}\n`,
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.on("open", async () => {
        ...
        try {
          ...
          if (errors.length === 0) {
            ...
            logTest(
              ...
              true,
              `All required fields present${warningMsg}`,
            );
          } else {
            logTest(
              `Message structure validation`,
      ...
      try {
        ...
        if (result1.result) {
          logTest(
            "RPC proxied successfully",
            true,
            `Block number: ${result1.result}`,
          );
        } else if (result1.error) {
          log(
            `    RPC provider returned error (this is OK): ${result1.error.message}`,
          );
          logTest(
            "RPC proxied successfully",
            true,
            "Request reached upstream provider (returned error)",
          );
        }
        ...
        try {
          await waitFor(
            () => state.messagesReceived.length > messageCountBefore,
            5000,
          );
          logTest(
            "WebSocket message received",
            true,
            "Message received after RPC call",
          );
        } catch (error) {
          logTest(
            "WebSocket message received",
            false,
            "No message received after RPC call",
          );
        }
        ...
        const result2 = await makeRPCCall("eth_getBalance", [
          testAddress,
          "latest",
        ]);
        ...
        if (result2.result !== undefined) {
          ...
          log(
            `    RPC provider returned error (this is OK): ${result2.error.message}`,
          );
          logTest(
            "RPC proxied successfully",
            true,
        ...
        try {
          await waitFor(
            () => state.messagesReceived.length > messageCountBefore2,
            5000,
          );
          const lastMessage =
            state.messagesReceived[state.messagesReceived.length - 1];

          if (lastMessage.log?.method === "eth_getBalance") {
            logTest(
              "WebSocket message contains correct method",
              true,
              "Method matches eth_getBalance",
            );

            if (
              lastMessage.log?.params?.[0]?.toLowerCase() ===
                testAddress.toLowerCase()
            ) {
              logTest(
                "WebSocket message contains correct address",
                true,
                "Address parameter matches",
              );
            } else {
              logTest(
                "WebSocket message contains correct address",
                false,
                "Address parameter does not match",
              );
            }
          } else {
            logTest(
              "WebSocket message contains correct method",
              false,
              `Expected eth_getBalance, got ${lastMessage.log?.method}`,
            );
          }
        } catch (error) {
          logTest(
